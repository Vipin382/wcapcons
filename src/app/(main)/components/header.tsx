...
const products: Product[] = [
  ...
  },
  {
    name: "Events",
    description:
      "Learn, Engage, Excel: Your Gateway to know parteners, team at CapCons",
    href: "#",
  ...
  {
    name: "Stormnorms",
    description: "Contest that celebrate creators",
    href: "#",
      ...
      >
        ...
        <PopoverGroup className="hidden lg:flex lg:gap-x-12">
          ...
          <Popover>
            ...
            >
              <PopoverPanel className="absolute inset-x-0 bg-background-950 top-0 -z-10 bg-background pt-14 shadow-lg ring-1 ">
                <div className="mx-auto grid max-w-7xl grid-cols-4 gap-x-4 px-6 py-10 lg:px-8 xl:gap-x-8">
                  {products.map((item: Product) => (
                    <div
                      key={item.name}
                      className="group relative rounded-lg p-6 text-sm leading-6 hover:bg-background-900/40"
                    >
                      <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-foreground-800 group-hover:bg-foreground">
                        <item.icon
                    ...
                    <div className="grid grid-cols-2 divide-x divide-gray-900/5 border-x border-gray-900/5">
                      {callsToAction.map((item: CallToAction) => (
                        <a
                          key={item.name}
                          href={item.href}
                          className="flex items-center justify-center gap-x-2.5 p-3 text-sm font-semibold leading-6 text-white"
                        >
                          <item.icon
                            className="h-5 w-5 flex-none text-gray-400"
                            aria-hidden="true"
